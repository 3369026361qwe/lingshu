...
class PerformanceMetrics:
    ...
    def compute_all(equity_curve: list[Decimal], daily_returns: list[Decimal], risk_free: Decimal = Decimal("0.03")) -> dict:
        ...
        return {
            "total_return": float(total_return.quantize(Decimal("0.0001"))),
            "annualized_return": float(ann_return.quantize(Decimal("0.0001"))) if ann_return else None,
            "annualized_vol": float(ann_vol.quantize(Decimal("0.0001"))) if ann_vol else None,
            "sharpe": float(sharpe.quantize(Decimal("0.0001"))) if sharpe else None,
            "max_drawdown": float(max_dd.quantize(Decimal("0.0001"))),
            "calmar": float(calmar.quantize(Decimal("0.0001"))) if calmar else None,
            "win_rate": float(win_rate.quantize(Decimal("0.0001"))),
            "n_days": len(daily_returns),
        }

    @staticmethod
    def sharpe_ratio(daily_returns: list[Decimal], risk_free: Decimal = Decimal("0.03")) -> Decimal | None:
    ...
    @staticmethod
    def _annualized_return(daily_returns: list[Decimal]) -> Decimal | None:
        if len(daily_returns) < 20:
            return None
    ...
    @staticmethod
    def _annualized_vol(daily_returns: list[Decimal]) -> Decimal | None:
        if len(daily_returns) < 20:
            return None
        n = Decimal(len(daily_returns))
        mean = sum(daily_returns) / n
